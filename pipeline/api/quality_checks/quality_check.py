...
# Canonical locations for Helm / container deployments.
# QUALITY_CHECKS_DIR defaults to /app/quality_checks but can be overridden with QUALITY_CHECKS_DIR.
QUALITY_CHECKS_DIR = Path(
    os.environ.get("QUALITY_CHECKS_DIR", "/app/quality_checks")
)
# Results directory: use RESULTS_DIR if set (e.g. in Docker), else a writable dir under the api package.
_default_results = Path(__file__).resolve().parent.parent / "data" / "results"
BASE_DIR = os.environ.get("RESULTS_DIR", str(_default_results))
...
# Config JSONs live under QUALITY_CHECKS_DIR (default /app/quality_checks).
dataset_filter_path = QUALITY_CHECKS_DIR / "dataset_variable_filter.json"
...
def quality_check(data, disease_type="sarcoma"):
    ...
    data_file = data
    repeated_entities_file = QUALITY_CHECKS_DIR / "repeteable_entities.json"
    suites_file = QUALITY_CHECKS_DIR / "expectations_data.json"
    app_path = BASE_DIR
    ...
    if df_long_all.empty:
        ...
        for _, r in focus.iterrows():
            ...
            if pd.isna(ph) or ph not in phase_stats:
                continue
    ...
    known_phases = {"Diagnosis", "Progression", "Recurrence"}
    for _, r in focus_rows.iterrows():
        if pd.isna(r.phase) or r.phase not in known_phases:
            continue
    ...
    filled_metadata_path = os.path.join(app_path, "filled_metadata.json")
    with open(filled_metadata_path, "w", encoding="utf-8") as fh:
        json.dump(filled, fh, ensure_ascii=False, indent=2)
    print(f"Saved: {filled_metadata_path}")
...
def build_missing_matrix_from_long_data(
    ...
    repeated_entities_file: str = str(QUALITY_CHECKS_DIR / "repeteable_entities.json"),
    entities_cardinality_file: str = str(QUALITY_CHECKS_DIR / "entities_cardinality.json"),
    variable_importance_file: str = str(QUALITY_CHECKS_DIR / "variable_importance.json"),
) -> pd.DataFrame:
    """
